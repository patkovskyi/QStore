...
namespace QSpell.Extensions
{
    ...
    {
        internal const int BINARY_SEARCH_THRESHOLD = 10;

        internal static int GetUpperIndex<T>(this IList<T> list, IList<Int32> lowerIndexes, Int32 lowerIndexIndex)
        {
        ...
        internal static int BinarySearch<T, S>(this IList<T> list, S item, Func<T, S> map, IComparer<S> comparer, int lower, int upper)
        {
            ...
            while (lower <= upper)
            {
                ...
                Int32 comparisonResult = comparer.Compare(map(list[middle]), item);
        ...
        internal static int BinarySearch<T>(this IList<T> list, T item, IComparer<T> comparer, int lower, int upper)
        {
            ...
            {
                Int32 middle = (lower + upper) / 2;
                Int32 comparisonResult = comparer.Compare(list[middle], item);
                if (comparisonResult == 0)
                    return middle;
                else if (comparisonResult > 0)
                    upper = middle - 1;
        ...
        internal static int OptimalSearch<T, S>(this IList<T> list, S item, Func<T, S> map, IComparer<S> comparer, int lower, int upper)
        {
            ...
            {
                return BinarySearch(list, item, map, comparer, lower, upper);
            }
            else
            {
                return LinearSearch(list, item, map, comparer, lower, upper);
            }
        }
        ...
        internal static int OptimalSearch<T>(this IList<T> list, T item, IComparer<T> comparer, int lower, int upper)
        {
            if (upper - lower > BINARY_SEARCH_THRESHOLD)
            {
                return BinarySearch(list, item, comparer, lower, upper);
            }
            else
            {
                return LinearSearch(list, item, comparer, lower, upper);
            }
        }        
